---
title: "FinalRExercises_ZhangZihao"
...
```{r}
library(tidyverse)
data_schools <- unite(data_schools, "key", county_name, year, sep = "", remove = FALSE)
data_acs <- unite(data_acs, "key", county_name, year, sep = "", remove = FALSE)
MergedData <- merge(data_schools, data_acs, by = "key", all.y = FALSE, all.x = TRUE)
MergedData2 <- left_join(data_schools, data_acs, by = "key")
rm(MergedData2)
```
...
MergedData %>% 
    ...
    summarize(total_enrollment = sum(total_enroll, na.rm = T), 
              percent1 = mean(per_free_lunch + per_reduced_lunch, na.rm = T), 
              percent2 = mean(county_per_poverty, na.rm = T)) %>% 
    ungroup()
...
MergedData1 <- MergedData %>% 
    filter(year.x == 2009) %>% 
    group_by(county_name.x) %>% 
    mutate(total_enrollment = sum(total_enroll, na.rm = T), 
              percent1 = mean(per_free_lunch + per_reduced_lunch, na.rm = T), 
              percent2 = mean(county_per_poverty, na.rm = T)) %>% 
    ungroup()
head(MergedData1[order(MergedData1$percent2, decreasing=TRUE),])
head(MergedData1[order(MergedData1$percent2, decreasing=FALSE),])
```
...
```{r}
MergedData %>% 
    filter(school_name == 'CLARKSVILLE ELEMENTARY SCHOOL') %>% 
    group_by(year.x) %>% 
    summarize(total_enrollment = sum(total_enroll, na.rm = T), 
              percent1 = mean(per_free_lunch + per_reduced_lunch, na.rm = T), 
              mean_score = mean(mean_math_score, na.rm = T)) %>% 
    ungroup() %>% 
    ggplot(mapping = aes(x = mean_score, y = percent1)) + geom_line() 

MergedData %>%
    group_by(loworhigh) %>% 
    summarize(test_perf = mean(mean_math_score + mean_ela_score, na.rm = T)) %>% 
    ggplot(mapping = aes(x = loworhigh, y = test_perf)) + geom_bar(stat = 'identity') 
```
